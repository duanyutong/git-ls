    ...
    G: GitBackend + ?Sized,
{
    ...
    let (lanes, main_oid, repository) = match build_lanes(git, args, &mut meta_cache)? {
        BuiltLanes::Empty {
            ...
            let now_timestamp = environment.now_timestamp();
            ...
            write_rendered_line(stdout, &render_top_spacer(&colours, false), environment)?;
            write_rendered_line(stdout, &render_main_tip(&ctx), environment)?;
            write_rendered_line(stdout, &render_omitted_main_past(&colours), environment)?;
            return Ok(());
        }
